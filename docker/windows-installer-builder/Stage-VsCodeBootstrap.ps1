...
param(
  [string]$RepoRoot = "",
  [string]$SourcePath = "C:\Users\sveld\Downloads\VSCodeSetup-x64-1.109.3.exe",
...
$bootstrapContract = $releaseContract.builderContract.runtimeBootstrapInstallers.vscode
...
if (-not (Test-Path -LiteralPath $sourcePathResolved)) {
  throw "Visual Studio Code bootstrap installer not found at $sourcePathResolved."
}
...
if ($actualName -ne $expectedName) {
  throw "Unexpected Visual Studio Code bootstrap installer name '$actualName'. Expected '$expectedName'."
}

$actualHash = Get-Sha256 -Path $sourcePathResolved
if ($actualHash -ne $expectedHash) {
  throw "Unexpected Visual Studio Code bootstrap installer hash. Expected $expectedHash but found $actualHash."
}
...
Write-Host "Staged Visual Studio Code bootstrap installer at $destinationPathResolved"
