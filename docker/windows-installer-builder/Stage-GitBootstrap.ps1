...
param(
  [string]$RepoRoot = "",
  [string]$SourcePath = "C:\Users\sveld\Downloads\Git-2.53.0-64-bit.exe",
...
$bootstrapContract = $releaseContract.builderContract.runtimeBootstrapInstallers.git
...
if (-not (Test-Path -LiteralPath $sourcePathResolved)) {
  throw "Git bootstrap installer not found at $sourcePathResolved."
}
...
if ($actualName -ne $expectedName) {
  throw "Unexpected Git bootstrap installer name '$actualName'. Expected '$expectedName'."
}

$actualHash = Get-Sha256 -Path $sourcePathResolved
if ($actualHash -ne $expectedHash) {
  throw "Unexpected Git bootstrap installer hash. Expected $expectedHash but found $actualHash."
}
...
Write-Host "Staged Git bootstrap installer at $destinationPathResolved"
