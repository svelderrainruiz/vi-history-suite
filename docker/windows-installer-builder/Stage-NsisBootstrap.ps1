...
param(
  [string]$RepoRoot = "",
  [string]$SourcePath = "C:\Users\sveld\Downloads\nsis-3.11-setup.exe",
...
$expectedName = $releaseContract.builderContract.toolchainReferences.nsis.bootstrapInstaller.fileName
$expectedHash = $releaseContract.builderContract.toolchainReferences.nsis.bootstrapInstaller.sha256

$sourcePathResolved = Resolve-PathOrLiteral -Path $SourcePath
if (-not (Test-Path -LiteralPath $sourcePathResolved)) {
  throw "NSIS bootstrap installer not found at $sourcePathResolved."
}

$destinationPathResolved = if ($DestinationPath) {
  [IO.Path]::GetFullPath($DestinationPath)
} else {
  Join-Path $repoRootPath "docker/windows-installer-builder/vendor/$expectedName"
}
...
if ($actualName -ne $expectedName) {
  throw "Unexpected NSIS bootstrap installer name '$actualName'. Expected '$expectedName'."
}

$actualHash = Get-Sha256 -Path $sourcePathResolved
if ($actualHash -ne $expectedHash) {
  throw "Unexpected NSIS bootstrap installer hash. Expected $expectedHash but found $actualHash."
}
...
Write-Host "Staged NSIS bootstrap installer at $destinationPathResolved"
